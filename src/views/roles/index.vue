<template>
  <div>
    ...
    <el-dialog
      ...
      width="50%"
      @close="editRoleDialogVisibleClose"
    >
      <span slot="footer" class="dialog-footer">
        <el-form
          :model="eidtForm"
...
export default {
  ...
  methods: {
    ...
    async showDialogRights (row) {
      ...
      const { data: res } = await rightsListApi('tree')
      if (res.meta.status !== 200) {
    ...
    async allotRights () {
      ...
      //   { rids: idStr }
      // )
      const { data: res } = await setRoleRights(this.roleId, params)
    ...
    ok () {
      if (this.dialogTitle === '新增角色') return this.addRole()
      this.editRole()
    },
    // 显示编辑角色对话框
    async showEditRole (row) {
      ...
      const { data: res } = await getRoleDetail(row.id)
      if (res.meta.status !== 200) {
    ...
    async editRole () {
      ...
      //   this.eidtForm
      // )
      const { data: res } = await setRole(this.eidtForm)
      if (res.meta.status !== 200) return this.$message.error(res.meta.msg)
    ...
    async addRole () {
      const { data: res } = await addRole(this.eidtForm)
